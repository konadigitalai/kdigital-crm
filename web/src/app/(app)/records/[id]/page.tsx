...
import { getCatalog, getRecord } from "@/lib/api";
...
export default async function RecordPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [data, catalog] = await Promise.all([getRecord(id), getCatalog()]);
  if (!data) notFound();
    ...
    <>
      ...
      >
        ...
        <div className="border-r border-rule px-9 pb-[60px] pt-7">
          ...
          {data.approval && <ApprovalCard approval={data.approval} />}

          {/* NBA — generated by the Next-Best-Action Agent (Claude). */}
          <NbaCard
            leadNumber={lead.number}
