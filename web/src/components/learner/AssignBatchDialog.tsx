...
import { assignLearnerToBatch, getBatches, updateEnrolmentStatus } from "@/lib/api";
import type { Batch, BatchSlot, EnrolmentStatus } from "@/lib/types";
...
// Status changer (used on each assignment row in the learner record)
export function EnrolmentStatusMenu({ partyId, enrolmentId, current }: {
  partyId: string; enrolmentId: string; current: EnrolmentStatus;
}) {
  const router = useRouter();
  const [busy, setBusy] = useState(false);
  const [open, setOpen] = useState(false);
  ...
  async function set(status: EnrolmentStatus) {
    ...
    setBusy(true);
    try {
      await updateEnrolmentStatus(partyId, enrolmentId, status);
      router.refresh();
    } finally {
      setBusy(false);
      setOpen(false);
